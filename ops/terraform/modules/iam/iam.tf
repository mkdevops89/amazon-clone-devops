...
resource "aws_iam_policy" "cost_explorer_policy" {
  name        = coalesce(var.cost_explorer_policy_name, "${title(var.project)}${title(var.environment)}CostExplorerPolicy")
...
resource "aws_iam_policy" "bedrock_invoke_policy" {
  name        = coalesce(var.bedrock_policy_name, "${title(var.project)}${title(var.environment)}BedrockInvokePolicy")
