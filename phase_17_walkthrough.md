...
### D. Layer 7 Dynamic Application Security Testing (DAST)

To prove our application is secure against OWASP Top 10 web vulnerabilities (like XSS or SQL Injection), we execute an automated DAST baseline scan using a headless OWASP ZAP Docker container directly against our live production URL.

**Test Command (The ZAP Scan):**
```bash
# Run a headless OWASP ZAP baseline scan against the live production Ingress endpoint
docker run -v $(pwd):/zap/wrk/:rw -t zaproxy/zap-stable zap-baseline.py -t https://devcloudproject.com -I
```

**Expected Result:**
The scan will aggressively spider the web application and inject various payloads. It will return an `Exit code: 0` (PASS), proving the React/Next.js frontend and Spring Boot backend are immune to baseline XSS and SQLi attacks.

---

### E. HTTP Security Headers & Anti-Clickjacking 

We configured the NGINX Ingress Controller to enforce strict HTTP Security Headers (HSTS) and prevent Clickjacking/MIME-sniffing. 

**Test Command (The Header Probe):**
```bash
curl -I https://devcloudproject.com
```

**Expected Result:**
The HTTP `200 OK` response will firmly include the following active security headers:
...
### F. API Gateway Zero-Trust Authentication Validation

We constructed a Spring Cloud API Gateway that enforces Amazon Cognito OAuth 2.0 JWT validation before allowing any traffic to reach the backend microservices. We will attempt to bypass this gateway.

**Test Command (The Unauthorized Request):**
```bash
# Attempt to request sensitive data from the backend without an active JWT token
curl -v https://api.devcloudproject.com/api/v1/orders
```

**Expected Result:**
The gateway will intercept the unauthenticated request, refuse to forward it to the internal cluster, and immediately throw an `HTTP/2 401 Unauthorized` error with a `www-authenticate: Bearer` challenge!
